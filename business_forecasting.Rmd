...
The goal of this project is to propose a machine learning model capable to predict the revenue **for a particular company**. We are going to build a forecasting tool that estimates the business revenue during the next months, until the end of the year. 

The data was collected annually from 2017 through the current year 2021. At this moment in time, the forecast numbers are estimated using naive methods, and we want to assess if a new model(s) can output better predictions. We have collected data **for the last 2 months (August / September)** to validate our performance.

## Business Value 

It’s common that we use traditional methods (exponential smoothing or moving average) to predict our **best estimate** revenue. Common operations in our business are highly composed by external or systematic situations that we cannot control and thus, our predictions can be highly affected as well. The reason for this, is that our models are too weak in terms of performance. 

In the present document, we are proposing a new tool capable to give better and accurate predictions, by using **advanced machine learning**.
...
The fist step is to get the data which has been stored in an Excel format. 

The basic steps are: Collect data, convert nomenclatures, convert time-based columns to date & filtering the data. The category “Forecast” contains the predictions from the traditional model. Our goal is to improve these numbers.
...
financials <- read_excel('/home/analytics/R/Projects/Time Series 2/00_data/data.xlsx',
...
fin_data_ac_rev_bus %>%
  plot_time_series(Date, Value, .smooth = FALSE, 
                   .facet_scales = "free",
                   .title = "Business Revenue (Actuals)" )  %>% layout(autosize = F)
...
By the middle of 2020, our revenue trend seems to be stabilized, compared to the previous years. Also, the revenue had been decreasing from the middle of 2017 to the middle of 2020 in about **27.6 %** :
...
fin_data_rev_bus %>% ungroup() %>%
  plot_time_series(Date, Value, .color_var = Category, .smooth = FALSE,
                   .title = "Business Revenue (Actuals vs Forecast)") %>% layout(autosize = F)
...
fin_data_ac_rev_bus %>%
  plot_stl_diagnostics(Date, log_interval_vec(Value, offset = 1000000), 
                       .facet_scales = "free_y",
                       .feature_set = c( "observed", "season", "trend", "remainder")) %>% layout(autosize = F)
...
fin_data_ac_rev_bus %>% 
  ...
  plot_seasonal_diagnostics(Date, log_interval_vec(Value, offset = 1000000),
                            .feature_set = "year") %>% layout(autosize = F)
...
fin_data_ac_rev_bus %>% 
  ...
  plot_seasonal_diagnostics(Date, log_interval_vec(Value, offset = 1000000),
                            .feature_set = "month.lbl") %>% layout(autosize = F)
...
Clearly, we can spot some seasonality on a monthly basis. As we can see on the previous graph, there is quite a lot of opportunity in the business during the last quarter of the year. We usually call it, "the busy months" since we are collecting most of the revenue during this period.
...
### 5.2 Data preparation: "The Recipe"
...
## 6. Modelling Stage (Sequential / Non-Sequential)

### 6.1. Modelling Stage 1 (Sequential models)
...
### 6.2. Modelling Stage 2 (Non-Sequential models)
...
resamples_kfold %>%
  tk_time_series_cv_plan() %>%
  plot_time_series_cv_plan(Date, value_trans, .facet_ncol = 2) %>% layout(autosize = F)
...
## 9. Proposed Models vs Traditional Model (Best Estimate)

Next step is to compare the results against the traditional forecasting method **(BEST ESTIMATE)**.

First, lets integrate the data.
...
ggplotly(table_forecast_metrics_ggplot) %>% layout(autosize = F)
...
## 10. Ensemble Model vs Traditional Model (Best Estimate)
...
ggplotly(table_forecast_ensemble_metrics_ggplot) %>% layout(autosize = F)
...
## 11. FINAL WORDS AND RECOMMENDATIONS


**(1)** According to our proposal, the ensemble model have improved the forecasting accuracy in about 63%. These results were compared against the traditional method, by taking the last 2 months (August / September 2021) of the data.
...
**(4)** Next steps are to publish the model using our tools such as POWER BI and Shiny and keep monitoring the performance during the next 2 months. Unless there are no changes in our current business structure, we shall continue with the proposal; otherwise, it’s recommended to run the process again and taking into account the new changes on the structure.
